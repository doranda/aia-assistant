// src/components/mpf/portfolio-track-record.tsx
// Displays the synthetic portfolio NAV from mpf_portfolio_nav
// This tracks ACTUAL performance since tracking began (not theoretical weighted returns)
...
interface PortfolioTrackRecordProps {
...
export function PortfolioTrackRecord({ navHistory, inceptionDate }: PortfolioTrackRecordProps) {
  ...
  const sinceInception = first.nav > 0
  ...
  const mtdReturn = mtdStart && mtdStart !== latest
  ...
  const ytdReturn = ytdFirst && ytdFirst !== latest
    ...
    <section aria-labelledby="track-record-heading" className="mb-8">
      <div className="flex items-center justify-between mb-2">
        ...
        {inceptionDate && (
          <span className="text-[10px] font-mono text-zinc-400">
            Tracking since {new Date(inceptionDate).toLocaleDateString("en-HK", { day: "numeric", month: "short", year: "numeric" })}
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-zinc-900/50 border border-zinc-800/60 rounded-lg p-4">
          ...
          {latest.is_cash && (
            <div className="text-[10px] font-mono text-amber-400 mt-1">In cash (settling)</div>
          )}
        </div>
